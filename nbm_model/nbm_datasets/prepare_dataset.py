...
def prepare_dataset(directory, out_directory, freq_accuracy=33.3, dt=0.003, overlap_spectro=0.2, w_pix=1024, annotations=True, 
                    audio_format='', keep_files_p=None):
    """
    ...
    if keep_files_p is not None:
        with open(keep_files_p, 'r') as f:
            keep_files = json.load(f)

        dir_keep_files = [e.replace(f'{top_dir}__', '').replace(extra_str_label, '') for e in keep_files \
                            if top_dir in e]
    ...
    if annotations:
        labels = create_label_dataset(directory, extra_str_label=extra_str_label, suppress_unID=True, is_csv=False) # is_csv = top_dir == 'mediae'
    else:
        labels = None

    for file in audio_files:
        
        # Is it an authorized file?
        filename = os.path.basename(file).replace('.mp3', '').replace('.wav', '').replace('.WAV', '')
        if (keep_files_p is not None) and (not filename in dir_keep_files):
            print(f'** File {filename} not included, going to next file **')
            continue
...
class File_Processor:
    ...
    def load(self):
        ...
        if sr != self.FREQ:
            ...
            temp_f = f'{self.filename}_{str(now.microsecond)}.{self.ext}'
            if self.ext == 'wav':
    ...
    def process_long_file(self, data, freq_accuracy, dt, overlap_spectro):
        ...
        if len(data) > max_l:
            ...
            for k in range(int(len(data) / max_l) + 1):
                ...
                if annotations_inc is not None:
