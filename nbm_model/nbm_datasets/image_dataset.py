...
class Img_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Temporary solution, until class 0 (non bird sound) is removed from preprocessing
        pos_idx = np.array(bird_ids) != 0
        bboxes, bird_ids = torch.Tensor(bboxes)[pos_idx], torch.Tensor(bird_ids)[pos_idx]
        ...
        if self.transform:
            ...
            bool_transform = np.random.randint(2, size=4) # [1] * 3 # np.random.randint(2, size=4)
            ...
            if bool_transform[1] == 1:
                freq_accuracy = 33.3
                cutting_freq = np.random.randint(500, 10000)
                b, a = signal.butter(1, 2 * np.pi * cutting_freq, 'low', analog=True)
                w, h = signal.freqs(b, a, worN=2 * np.pi * (500 + np.arange(375) * freq_accuracy))
                # Addition of gain in log space
                mat = torch.Tensor(0.5 * np.log10(np.clip(abs(h), 1e-9, None))).unsqueeze(0).repeat(1024, 1).transpose(0, 1)
                img = img + mat
